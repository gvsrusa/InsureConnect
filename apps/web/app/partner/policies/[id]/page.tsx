...
export const metadata = { title: "Policy Detail — Partner Portal" };
...
export default async function PartnerPolicyDetailPage({ params }: Props): Promise<React.JSX.Element> {
  const { id } = await params;
  const token = await requireAuth("/login?portal=partner");

  // TODO: Remove fallback once GET /api/v1/admin/policies/:id is confirmed working
  let policy: Policy | undefined = MOCK_POLICIES.find((p) => p.id === id);
  try {
    policy = await partnerApi.policy(id, token);
  } catch (err) {
  ...
  if (!policy) {
    return (
      <div className="py-20 text-center">
        ...
        <Link href="/partner/policies" className="mt-3 inline-block text-sm text-pine hover:underline">
          ← Back to policies
        </Link>
      </div>
    );
